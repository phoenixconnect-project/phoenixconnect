...
#if defined(NDEBUG)
# error "Phoenixconnect cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Phoenixconnect: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Phoenixconnect Signed Message:\n";
...
{
    // Phoenixconnect: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
	...
	// bigger rewards to first 5000 blocks
	if (nHeight <= 5000)
		return 200 * COIN;
	
    // Subsidy is cut in half every 500000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 500000); // Phoenixconnect: 500k blocks in ~4.5 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Phoenixconnect: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Phoenixconnect: 120 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Phoenixconnect: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Phoenixconnect: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc1, 0xb7, 0xdc }; // Phoenixconnect: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PhoenixconnectMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PhoenixconnectMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PhoenixconnectMiner(CWallet *pwallet)
{
    printf("PhoenixconnectMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("phoenixconnect-miner");
    ...
    try { loop {
        ...
        printf("Running PhoenixconnectMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PhoenixconnectMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PhoenixconnectMiner, pwallet));
}
